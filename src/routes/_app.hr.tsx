...
export const Route = createFileRoute("/_app/hr")({
  head: () => ({
    meta: [
      { title: "Human Resources — EasyConstruct" },
      {
        name: "description",
        content:
          "Workforce, attendance, payroll, and AI-assisted HR intelligence for construction operations.",
      },
      { property: "og:title", content: "Human Resources — EasyConstruct" },
      {
        property: "og:description",
        content: "Manage employees, attendance, payroll, and workforce capacity in one workspace.",
      },
    ],
  }),
  component: HRPage,
...
function OverviewSection() {
  const kpis = [
    { label: "Total employees", value: "518", delta: "+12 this month", up: true, icon: Users },
    { label: "Active today", value: "478", delta: "92.3% attendance", up: true, icon: UserCheck },
    { label: "Absent / on leave", value: "40", delta: "-6 vs yesterday", up: true, icon: UserX },
    { label: "Open attendance issues", value: "14", delta: "+3 needs review", up: false, icon: AlertTriangle },
    { label: "Payroll cycle", value: "73%", delta: "Cut-off in 2 days", up: true, icon: Wallet },
    { label: "Avg attendance rate", value: "94.8%", delta: "+1.2 pts MoM", up: true, icon: CalendarCheck },
  ];
              ...
              {attendanceWeek.map((d) => {
                ...
                return (
                  <div key={d.day} className="flex flex-1 flex-col items-center gap-2">
                    ...
                    <div className="text-[10px] text-muted-foreground/70">{total}</div>
                  </div>
                );
              })}
            </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card className="rounded-2xl">
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            <Badge variant="secondary" className="rounded-full">Period · Jun 1–15</Badge>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <div className="flex items-end justify-between">
                <div>
                  <div className="text-3xl font-semibold tracking-tight">$2.84M</div>
                  <div className="text-xs text-muted-foreground">Gross labor (period)</div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-medium text-success">+4.1%</div>
                  <div className="text-[11px] text-muted-foreground">vs last period</div>
                </div>
              </div>
        ...
        <Card className="rounded-2xl">
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">Latest changes across people ops</p>
          </CardHeader>
          <CardContent className="space-y-3">
            {[
              { icon: UserCheck, text: "Adaeze Nwosu verified site attendance", time: "6 min", tone: "success" as const },
              { icon: AlertTriangle, text: "Geofence breach flagged — M. Bell", time: "22 min", tone: "warning" as const },
              { icon: Wallet, text: "Payroll batch B-118 sent for approval", time: "1 hr", tone: "info" as const },
              { icon: Plus, text: "2 new employees onboarded to Field Ops", time: "3 hr", tone: "muted" as const },
              { icon: ShieldCheck, text: "Document expiration — 4 certifications", time: "Today", tone: "warning" as const },
            ].map((a, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className={`mt-0.5 flex h-7 w-7 items-center justify-center rounded-lg ${toneBg(a.tone)}`}>
                  <a.icon className="h-3.5 w-3.5" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm">{a.text}</p>
                  <p className="text-[11px] text-muted-foreground">{a.time} ago</p>
                </div>
              </div>
            ))}
          </CardContent>
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {filtered.length} of {employees.length} employees · bulk actions available
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="rounded-xl">Archive</Button>
            <Button variant="outline" size="sm" className="rounded-xl">Reassign</Button>
          </div>
        </CardHeader>
  ...
  return (
    <TableRow className="hover:bg-muted/40">
      ...
      <TableCell className="text-right">
        <span className={e.attendanceRate >= 95 ? "text-success" : e.attendanceRate >= 85 ? "text-warning" : "text-destructive"}>
          {e.attendanceRate}%
        </span>
      </TableCell>
      <TableCell className="text-right text-sm">{e.performance.toFixed(1)}</TableCell>
      <TableCell>
        <DropdownMenu>
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <KpiMini label="Verified today" value="436" tone="success" icon={CheckCircle2} />
        <KpiMini label="Pending verification" value="28" tone="warning" icon={Clock} />
        <KpiMini label="Geofence flags" value="9" tone="destructive" icon={MapPin} />
        <KpiMini label="Photo auth failures" value="5" tone="destructive" icon={Camera} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <Card className="rounded-2xl xl:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" variant="outline" className="rounded-xl">Bulk verify</Button>
              <Button size="sm" className="rounded-xl">
              ...
              <TableBody>
                {attendanceLogs.map((l) => (
                  <TableRow key={l.empId} className="hover:bg-muted/40">
                    ...
                    <TableCell className="text-sm text-muted-foreground">{l.site}</TableCell>
        ...
        <Card className="rounded-2xl">
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">Attendance density · last 14 days</p>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {["Westgate Tower", "Harborline Hub", "Northgate Plaza", "Phoenix HQ"].map((site) => (
                <div key={site}>
...
function PayrollSection() {
  return (
    <>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <KpiMini label="Gross labor (period)" value="$2.84M" tone="info" icon={Wallet} />
        <KpiMini label="Net payable" value="$2.28M" tone="success" icon={CheckCircle2} />
        <KpiMini label="Overtime cost" value="$184K" tone="warning" icon={Clock} />
        <KpiMini label="Awaiting approval" value="124" tone="warning" icon={AlertTriangle} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <Card className="rounded-2xl xl:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Pay period Jun 1 – Jun 15 · 518 employees</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <TableBody>
                {payrollRows.map((p) => (
                  <TableRow key={p.empId} className="hover:bg-muted/40">
                    ...
                    <TableCell className="text-right text-sm">${p.gross.toLocaleString()}</TableCell>
        ...
        <Card className="rounded-2xl">
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">Distribution & checkpoints</p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-xl border bg-muted/30 p-4">
              <div className="text-xs text-muted-foreground">Total payable</div>
              <div className="mt-1 text-2xl font-semibold tracking-tight">$2,284,910</div>
              <div className="mt-2 text-[11px] text-success">+4.1% vs prev period</div>
            </div>
            <div className="space-y-3">
              {[
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <KpiMini label="Assigned today" value="418" tone="success" icon={UserCheck} />
        <KpiMini label="Available" value="62" tone="warning" icon={Activity} />
        <KpiMini label="Overtime crews" value="17" tone="destructive" icon={Clock} />
      </div>

      <Card className="rounded-2xl">
        <CardHeader>
          <CardTitle className="text-base">Workforce allocation board</CardTitle>
          <p className="text-xs text-muted-foreground">Per-site utilization, availability & overtime exposure</p>
        </CardHeader>
        <CardContent className="space-y-4">
          {sites.map((s) => {
              ...
              <div key={s.name} className="rounded-xl border p-4">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {s.assigned} assigned · {s.available} available · {s.ot} on overtime
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge
                      ...
                      className={`rounded-full ${
                        util > 90 ? "border-warning/40 text-warning" : "border-success/30 text-success"
                      }`}
                    >
                      {util}% utilization
                    </Badge>
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-3">
                  <Capacity label="Assigned" value={s.assigned} max={s.capacity} tone="primary" />
                  <Capacity label="Available" value={s.available} max={s.capacity} tone="info" />
                  <Capacity label="Overtime" value={s.ot} max={20} tone="warning" />
                </div>
              </div>
            );
          })}
...
function ReportsSection() {
  const reports = [
    { title: "Attendance report", desc: "Daily, weekly, monthly attendance breakdown with late-arrival analysis.", icon: CalendarCheck, tag: "Operations" },
    { title: "Payroll report", desc: "Gross labor, deductions, and net payable per period with approval trail.", icon: Wallet, tag: "Finance" },
    { title: "Workforce report", desc: "Capacity, allocation, and utilization across sites and departments.", icon: Users, tag: "Capacity" },
    { title: "Performance report", desc: "Employee performance trends, ratings, and outliers.", icon: TrendingUp, tag: "People" },
    { title: "HR analytics", desc: "Turnover, tenure, and engagement signals with AI explainability.", icon: Sparkles, tag: "Analytics" },
    { title: "Executive HR brief", desc: "C-level summary covering workforce health and labor cost trends.", icon: FileBarChart2, tag: "Executive" },
  ];
  return (
    <>
          ...
          <Card key={r.title} className="rounded-2xl">
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                ...
                <Badge variant="outline" className="rounded-full text-[10px]">{r.tag}</Badge>
              </div>
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">HR alerts requiring action</p>
        </CardHeader>
        <CardContent className="space-y-3">
          {[
            { icon: AlertTriangle, tone: "warning" as const, title: "Geofence breach — Marcus Bell", time: "22 min ago" },
            { icon: Wallet, tone: "info" as const, title: "Payroll batch B-118 awaiting approval", time: "1 hr ago" },
            { icon: ShieldCheck, tone: "warning" as const, title: "4 certifications expiring within 14 days", time: "Today" },
            { icon: Bell, tone: "destructive" as const, title: "Workforce risk: Westgate Tower understaffed Thu", time: "Today" },
          ].map((n, i) => (
            <div key={i} className="flex items-start justify-between rounded-xl border bg-muted/20 px-4 py-3">
              <div className="flex items-start gap-3">
                <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${toneBg(n.tone)}`}>
              ...
              <Button size="sm" variant="ghost" className="rounded-lg">Review</Button>
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Advisory — review before acting</p>
          </div>
        </div>
        <Badge variant="outline" className="rounded-full border-ai/30 text-ai">Live</Badge>
      </CardHeader>
      <CardContent className="space-y-3">
        {aiInsights.map((i) => (
          <div key={i.title} className="rounded-xl border border-ai/15 bg-background/60 p-3">
            ...
            <div className="mt-2 flex items-center justify-between gap-2">
              <div className="text-[11px] text-muted-foreground">
                <span className="font-medium text-foreground">Impact:</span> {i.impact}
              </div>
              <Button size="sm" variant="ghost" className="h-7 rounded-lg px-2 text-xs text-ai hover:bg-ai-soft">
...
function MiniStat({ label, value, tone }: { label: string; value: string; tone: "success" | "warning" | "destructive" }) {
  const color =
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant="outline" className={`rounded-full text-[10px] ${map[status] ?? ""}`}>
      {status}
    </Badge>
  );
}
...
function Capacity({
  ...
  const bg = tone === "primary" ? "bg-primary" : tone === "info" ? "bg-info" : "bg-warning";
  return (
...
function toneBg(tone: "success" | "warning" | "destructive" | "info" | "muted") {
  switch (tone) {
    case "success":
